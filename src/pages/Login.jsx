...
function Login() {
  ...
  useEffect(() => {
    ...
    const token = localStorage.getItem("relawanRohmi_token");
    if (token) {
  ...
  useEffect(() => {
    async function fetchSanityData() {
      try {
        const sanityData = await sanityClient.fetch(`*[_type == 'user-relawan-rohmi']{
          _id,
          name,
          email,
          type,
  ...
  const handleLogin = (event) => {
    ...
    const user = serverData.data.find(
      (relawanRohmi_userData) => relawanRohmi_userData.email === username && relawanRohmi_userData.password === password
    );
    ...
    if (user) {
      ...
      localStorage.setItem("relawanRohmi_token", token);
      localStorage.setItem("relawanRohmi_user", user.name);
      localStorage.setItem("relawanRohmi_id", user._id);
      localStorage.setItem('relawanRohmi_userData', JSON.stringify(filteredData));
